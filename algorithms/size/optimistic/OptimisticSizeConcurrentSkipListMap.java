package algorithms.size.optimistic;
 ...
 *
 * This file applies the size transformation using the optimistic methodology
...
import algorithms.size.optimistic.core.OptimisticSizeCalculator;
 ...
 * @since 1.6
 */
public class OptimisticSizeConcurrentSkipListMap<K, V> implements SizeSet<K, V> {
    /*
    ...
    @Contended
    private final transient OptimisticSizeCalculator sizeCalculator;
        ...
        for (;;) {
            ...
            if (b != null) {
                ...
                for (;;) {                       // find insertion point
                    ...
                    if (c < 0) {
                        sizeCalculator.blockSize();
                        try {
                            if (NEXT.compareAndSet(b, n,
                                    p = new Node<K,V>(key, value, n))) {
                                z = p;
                                sizeCalculator.updateMetadata(UpdateOperations.OpKind.INSERT);
                                break;
                            }
                        } finally {
                            sizeCalculator.unblockSize();
                        }
                    }
                }
                ...
                if ((n = b.next) == null)
                    ...
                    try {
                        if (VAL.compareAndSet(n, v, null)) {
                            result = v;
                            if (result != null) {
                                sizeCalculator.updateMetadata(UpdateOperations.OpKind.REMOVE);
                            }
                            unlinkNode(b, n);
     ...
     */
    public OptimisticSizeConcurrentSkipListMap(int maxTries) {
        this.comparator = null;
        sizeCalculator = new OptimisticSizeCalculator(maxTries);
    }
     ...
     */
    public OptimisticSizeConcurrentSkipListMap(Comparator<? super K> comparator,int maxTries) {
    ...
    static {
        try {
            ...
            HEAD = l.findVarHandle(OptimisticSizeConcurrentSkipListMap.class, "head",
                    Index.class);
