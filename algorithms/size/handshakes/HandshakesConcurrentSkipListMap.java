package algorithms.size.handshakes;
 ...
 * to the class `algorithms.baseline.ConcurrentSkipListMap`,
 * a variant of `java.util.concurrent.ConcurrentSkipListMap` by Doug Lea.
 *
 * The transformation is based on the wait-free size implementation 
 * `algorithms.size.sp.SizeConcurrentSkipListMap` from the paper "Concurrent Size"
...
import algorithms.size.handshakes.core.HandshakeSizeCalculator;
import algorithms.size.handshakes.core.SizePhases;
 ...
 * @since 1.6
 */
public class HandshakesConcurrentSkipListMap<K, V> implements SizeSet<K, V> {
    /*
    ...
    @Contended
    private final transient HandshakeSizeCalculator sizeCalculator = new HandshakeSizeCalculator();
    ...
    /**
     * Complete remove operation for marked node in slow path
     * 
     * @param b predecessor node
                ...
                if ((d = q.down) != null) q = d;
                    ...
                    if ((b = q.node) != null) {
                        while ((n = b.next) != null) {
                            int c;
                            K k = n.key;
                            if (k == null || (c = cpr(cmp, key, k)) > 0) {
                                // Cannot simply advance b in case remove info is installed in n.valOrRemoveInfo
                                // similarly to ConcurrentSkipListMap,
                                // because then if c==0, the removal of n must be reported to sizeCalculator
    ...
    /**
     * Add indices after an insertion in the slow path. Descends iteratively to the
     ...
     */
    static <K, V> boolean slow_addIndices(Index<K, V> q, int skips, Index<K, V> x, Comparator<? super K> cmp) {
        Node<K, V> z;
        K key;
            ...
            for (;;) { // find splice point
                ...
                if (c < 0) {
                    if ((d = q.down) != null && skips > 0) {
                        --skips;
                        q = d;
                    } else if (d != null && !retrying && !slow_addIndices(d, 0, x.down, cmp)) break;
                    else {
                        x.right = r;
    ...
    /**
     * Add indices after an insertion in the fast path. Descends iteratively to the
     * highest level of insertion, then recursively, to chain index
     * nodes to lower ones. Returns null on (staleness) failure,
     * disabling higher-level insertions. Recursion depths are
     * exponentially less probable.
    ...
    /**
     * Slow path insertion method. Adds element if not present, or
     * replaces value if present and onlyIfAbsent is false.
     */
            ...
            if (b != null) {
                ...
                for (;;) { // find insertion point
                    ...
                    if ((n = b.next) == null) {
                        ...
                        // In case n.val was CASed and n's insertion was not yet linearized then, the
    ...
    /**
     * Fast path insertion method. Adds element if not present, or
     * replaces value if present and onlyIfAbsent is false.
     */
    ...
    /**
     * Slow path deletion method. Locates node, nulls value, appends a
     * deletion marker, unlinks predecessor, removes associated index
     * nodes, and possibly reduces head index level.
     */
    private final V slow_doRemove(Object key, Object value) {
        if (key == null) throw new NullPointerException();
        Comparator<? super K> cmp = comparator;
        V result = null;
        Node<K, V> b;
        ...
        while ((b = findPredecessor(key, cmp)) != null && result == null) {
            for (;;) {
                ...
                if ((n = b.next) == null) break outer;
                    ...
                    // Either n's insert is linearized and so n.val==value is linearized too, or n's
                    // insert is not linearized.
                    // Thus, the remove should anyhow fail, and there is no need to call
                    // updateMetadataAndLinearize for the insert.
                    break outer;
                else {
                    UpdateInfo insertInfo = n.insertInfo;
                    if (insertInfo != null) {
    ...
    /**
     * Fast path deletion method. Locates node, nulls value, appends a
     * deletion marker, unlinks predecessor, removes associated index
     * nodes, and possibly reduces head index level.
     */
    private final V fast_doRemove(Object key, Object value) {
        if (key == null) throw new NullPointerException();
        Comparator<? super K> cmp = comparator;
        V result = null;
        Node<K, V> b;
        ...
        while ((b = findPredecessor(key, cmp)) != null && result == null) {
            for (;;) {
                ...
                else if (value != null && !value.equals(valOrRemoveInfo)) break outer;
                else if (VAL_OR_REMOVE_INFO.compareAndSet(n, valOrRemoveInfo, null)) {
                    result = (V) valOrRemoveInfo;
                    unlinkNode(b, n);
                    break; // loop to clean up
                }
     ...
     */
    public HandshakesConcurrentSkipListMap() {
     ...
     */
    public HandshakesConcurrentSkipListMap(Comparator<? super K> comparator) {
     ...
     * or {@code null} if this map uses the {@linkplain Comparable natural ordering} of its keys.
     *
    ...
    static {
        try {
            ...
            HEAD = l.findVarHandle(HandshakesConcurrentSkipListMap.class, "head", Index.class);
    ...
    /**
    * For debug purposes - returns sum of all keys in the map.
    */
    public long getSumOfKeys() {
        ...
        while ((n = b.next) != null) {
            K k = n.key;
            if (k != null && (n.valOrRemoveInfo != null && !(n.valOrRemoveInfo instanceof UpdateInfo)))
                keysSum += (Integer) k;
            b = n;
        }
        return keysSum;
