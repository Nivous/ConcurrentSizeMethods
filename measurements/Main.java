...
import measurements.adapters.AbstractAdapter;
import measurements.support.*;
import measurements.support.zipfian.ScrambledZipfianGenerator;
...
public class Main {
    ...
    protected Integer retryParam;
    protected boolean isSplit;

    // some timing variables
    protected AtomicLong startUserTime = new AtomicLong(0);
    protected AtomicLong startWallTime = new AtomicLong(0);
    protected boolean useZipfian = false;
    protected double zipfianTheta = 0.99; // default (YCSB-style)


    public Main(int nthreads, int numOfSizeWorkers, int ntrials, double nseconds, int sizeDelay, String filename,
            PercentageRatio workloadRatio, String alg, int initSize, boolean prefill, Integer setParam, Integer retryParam, boolean isSplit, boolean useZipfian, double  zipfianTheta)  {
        this.nthreads = nthreads;
        this.numOfSizeWorkers = numOfSizeWorkers;
        this.ntrials = ntrials;
        this.nseconds = nseconds;
        this.sizeDelay = sizeDelay;
        ...
        this.useZipfian = useZipfian;
        this.zipfianTheta = zipfianTheta;
    }
    ...
    public static void invokeRun(String[] args) {
        ...
        Integer retryParam = null;
        boolean isSplit = false;
        boolean useZipfian = false;
        ...
        try {
            ...
            sizeDelay = Integer.parseInt(args[4]);
            alg = args[5];
        } catch (Exception ex) {
        ...
        if (sizeDelay < 0) {
            System.out.println("ERROR: Number of time delay for size operations must satisfy 0 <= ns");
            System.exit(-1);
        }
        ...
        for (String arg : args) {
            if (arg.startsWith("-")) {
                if (arg.matches("-ins[0-9]+")) {
                    ...
                    try {
                        retryParam = Integer.parseInt(arg.substring("-retry-".length()));
                    } catch (Exception ex) {
                        System.out.println("ERROR: The retry parameter must be a 32-bit integer.");
                        System.exit(-1);
                    }
        ...
        // }
        (new Main(numOfWorkloadWorkers + numOfSizeWorkers, numOfSizeWorkers, ntrials, nseconds, sizeDelay, filename,
                new PercentageRatio(insPercent, remPercent, 0),
                alg, initSize, prefill, setParam, retryParam, isSplit, useZipfian, zipfianTheta)).run();
    }
    ...
    protected boolean runTrial(
        ...
        ArrayList<KeyGenerator> arrays = ex.factory.getGenerators(ex.maxKey, rng); // key generators supply keys for each thread
        ArrayList<KeyGenerator> arrays_uniform = ex.factory.getGenerators_uniform(ex.maxKey, rng); // key generators supply keys for each thread
        ...
        for (int i = 0; i < nthreads - numOfSizeWorkers; i++) { // workload threads
            workers.add(new TimedWorker<>(arrays.get(i),arrays_uniform.get(i), ex, ex.workloadRatio, new Random(rng.nextInt()), set, start, startUserTime, startWallTime, i, isSplit));
        }
        for (int i = nthreads - numOfSizeWorkers; i < nthreads; i++) { // size threads
            workers.add(new TimedWorker<>(arrays.get(i),arrays_uniform.get(i), ex, new PercentageRatio(0, 0, 100), new Random(rng.nextInt()), set, start, startUserTime, startWallTime, i, isSplit));
        }
        ...
        try {
            for (int i = 0; i < nthreads; i++) workers.get(i).join();
    ...
    protected ArrayList<Experiment> getExperiments() {
        ...
        KeyGeneratorFactory keyGen = new KeyGeneratorFactory(useZipfian, zipfianTheta);
        exp.add(new Experiment(alg, setParam, retryParam, initSize, workloadRatio, keyGen));
        return exp;
    }
    ...
    public void run() {
        ...
        for (Experiment ex : exp) {
            ...
            // and run the trial
            for (SetFactory<Integer> factory : Factories.factories)
                if (ex.alg.equals(factory.getName())) {
                    stdout.println("Running " + ex);
                    for (int trial = 0; trial < ntrials; ++trial) {
                        stdout.print(".");
                        Camera.camera.timestamp = 0;
                        System.gc();
                        SetInterface<Integer> set = factory.newSet(ex.param,retryParam);
                        ...
                        if (name.contains("Optimistic") && ex.retryParam != null) {
                            name += "-Retry"+ex.retryParam;
                        }
                        if (!runTrial(out, name + "," + trial, p, experimentRng, (AbstractAdapter<Integer>) set, ex, isSplit))
                            System.exit(-1);
                    }
                    stdout.println("");
                }
    ...
    /**
     * A key generator that produces keys according to a Zipfian distribution.
     * It uses the ScrambledZipfianGenerator from the YCSB benchmark suite.
     */
    public class ZipfianKeyGenerator implements KeyGenerator {
        private ScrambledZipfianGenerator scrambledZipfianGenerator;
    
        public ZipfianKeyGenerator(int min, long max, double zipfianconstant) {
            this.scrambledZipfianGenerator = new ScrambledZipfianGenerator(min, max, zipfianconstant);
        }

        @Override
        public int next() {
            return Math.toIntExact(this.scrambledZipfianGenerator.nextValue());
        }
    }

    public static final class UniformKeyGenerator implements KeyGenerator{
        final Random rng;
        final int maxKey;

        public UniformKeyGenerator(final Random rng, final int maxKey) {
            this.rng = rng;
    ...
    static final class FixedNumberOfKeysWorker<K extends Comparable<? super K>> extends Thread {
        ...
        public FixedNumberOfKeysWorker(
            ...
            this.keyGen = new UniformKeyGenerator(rng, maxKey);
    ...
    protected static final class PercentageRatio {
        ...
        public PercentageRatio(final int ins, final int del, final int size) {
            if (ins < 0 || del < 0 || size < 0 || ins + del + size > 100)
                throw new RuntimeException("invalid percentageRatio " + ins + "i-" + del + "d-" + size + "size");
            this.del = del;
            this.ins = ins;
            this.size = size;
        }
        ...
        public String toString() {
            return "" + ins + "i-" + del + "d-" + (100 - ins - del - size) + "f";
        }
    }
    ...
    public class KeyGeneratorFactory {
        private final boolean useZipfian;
        private final double theta;
    
        public KeyGeneratorFactory(boolean useZipfian, double theta) {
            this.useZipfian = useZipfian;
            this.theta = theta;
        }
    
        public ArrayList<KeyGenerator> getGenerators(int maxKey, java.util.Random rng) {
            ArrayList<KeyGenerator> generators = new ArrayList<>(nthreads);
            for (int i = 0; i < nthreads; i++) {
                Random threadRng = new Random(rng.nextInt());
                if (useZipfian) {
                    generators.add(new ZipfianKeyGenerator(1,maxKey,theta));
                } else {
                    generators.add(new UniformKeyGenerator(threadRng, maxKey));
                }
            }
            return generators;
        }
        public ArrayList<KeyGenerator> getGenerators_uniform(int maxKey, java.util.Random rng) {
            ArrayList<KeyGenerator> generators = new ArrayList<>(nthreads);
            for (int i = 0; i < nthreads; i++) {
                Random threadRng = new Random(rng.nextInt());
                generators.add(new UniformKeyGenerator(threadRng, maxKey));
    ...
    public class TimedWorker<K extends Comparable<? super K>> extends Worker {
        ...
        public TimedWorker(final KeyGenerator keyGen,
                           final KeyGenerator keyGenUniform,
                           final Experiment ex,
                           final PercentageRatio percentageRatio,
                           final Random rng,
                           final AbstractAdapter<K> set,
            ...
            this.isSplit = isSplit;
            this.keyGen = keyGen;
            this.keyGenUniform = keyGenUniform;
        }
        ...
        private void executeWorkloadOp(double op) {
            if (op < percentageRatio.size + percentageRatio.ins) {
                final Integer keyInt = keyGenUniform.next();
                ...
                // This key generator can be either Zipfian or Uniform
        ...
        public long getTrueIns() {
            return trueIns;
        }

        public long getFalseIns() {
            return falseIns;
        }

        public long getTrueDel() {
            return trueDel;
        }

        public long getFalseDel() {
            return falseDel;
        }

        public long getTrueFind() {
            return trueFind;
        }

        public long getFalseFind() {
            return falseFind;
        }
    ...
    public final class Experiment {
        final String alg;
        final Integer param;
        final Integer retryParam;
        final int initSize;
        final int maxKey;
        final PercentageRatio workloadRatio;
        final KeyGeneratorFactory factory;
        volatile boolean isRunning = false;

        public Experiment(final String alg, final Integer param, Integer retryParam, final int initSize, final PercentageRatio workloadRatio, final KeyGeneratorFactory factory) {
            this.alg = alg;
            this.param = param;
            this.retryParam = retryParam;
            this.initSize = initSize;
        ...
        @Override
        public String toString() {
            return alg +(retryParam == null ? "" : ("-Retry"+retryParam))+ (param == null ? "" : ("-" + param)) + "-" + (nthreads - numOfSizeWorkers) + "workloadThreads-" + numOfSizeWorkers + "sizeThreads-" + initSize + "setSize-" + workloadRatio;
        }
    }
}
