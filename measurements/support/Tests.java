...
public class Tests {
    ...
    static final class ContainsAndSizeWorker extends Thread {
        ...
        public void run() {
            ...
            int size = set.size();
            // if (foundKeys > size) System.out.println("foundKeys=" + foundKeys + " size="
            // + size);
    ...
    static long fill(AbstractAdapter<Integer> set, int numKeys, int maxKey, java.util.Random rand,
            final boolean isSizeSupported, final boolean separateRanges) {
        long keysum = 0;
        ...
        for (int i = 0; i < NUM_THREADS; i++) {
            ...
            if (separateRanges) {
                // create a separate key range for each thread, so that an equal number of
                // elements are filled in each range and no range gets full, so that inserts in
                // following tests may succeed
                minKeyForThread = 1 + i * rangeSize;
                maxKeyForThread = (i + 1) * rangeSize;
            }
            fillerWorkers[i] = new FixedNumberOfKeysWorker((AbstractAdapter) set, numKeys / NUM_THREADS,
                    minKeyForThread, maxKeyForThread, new Random(rand.nextInt()), i);
        }
    ...
    static long empty(AbstractAdapter<Integer> set, int numKeysToRemove, int initialSize, int maxKey,
            java.util.Random rand, final boolean isSizeSupported) {
        long keysum = 0;
        ...
        for (int i = 0; i < NUM_THREADS; i++) {
            emptyingWorkers[i] = new EmptyingWorker((AbstractAdapter) set, numKeysToRemove / NUM_THREADS, maxKey,
                    new Random(rand.nextInt()), i);
        }
    ...
    static long insertRemoveAlternately(AbstractAdapter<Integer> set, int initialSize, int maxKey,
            java.util.Random rand, final boolean isSizeSupported, final boolean separateRanges) {
        long keysum = 0;
        ...
        for (int i = 0; i < NUM_THREADS; i++) {
            ...
            int maxKeyForThread = maxKey;
            if (separateRanges) {
                // create a separate key range for each thread, so that it can anticipate the
                // result of contains following an insert or remove.
                minKeyForThread = 1 + i * rangeSize;
                maxKeyForThread = (i + 1) * rangeSize;
            }
            alternatelyInsertRemoveWorkers[i] = new AlternatelyInsertRemoveWorker((AbstractAdapter) set,
                    minKeyForThread, maxKeyForThread, new Random(rand.nextInt()), i, separateRanges);
        }
        if (isSizeSupported) {
            sizeRangeVerifier = new SizeRangeVerifier((AbstractAdapter) set, initialSize, initialSize + NUM_THREADS,
                    NUM_THREADS);
        }
        ...
        for (int i = 0; i < NUM_THREADS; i++)
            alternatelyInsertRemoveWorkers[i].start();
        ...
        try {
            for (int i = 0; i < NUM_THREADS; i++)
                alternatelyInsertRemoveWorkers[i].join();
        } catch (InterruptedException e) {
            e.printStackTrace();
            System.exit(-1);
        }
    ...
    // This is the example in our paper's introduction, which exposes that
    // java.util.concurrent.ConcurrentSkipListMap is not linearizable
        ...
        for (int i = 0; i < TARGET_SIZES_PER_THREAD.length; ++i) {
            for (int j = 0; j < MAX_KEY_RATIOS.length; ++j) {
                ...
                if (maxKey < targetTotalSize + 1) {
                    ...
                    // Otherwise, it will be impossible to insert when the set is of size
                ...
                assert fill(set, targetTotalSize, maxKey, experimentRng, isSizeSupported, false) +
                        insertRemoveAlternately(set, targetTotalSize, maxKey, experimentRng, isSizeSupported,
                                false) == empty(set, targetTotalSize, targetTotalSize, maxKey, experimentRng,
                                        isSizeSupported);
                if (isSizeSupported)
                    assert set.size() == 0;
                ...
                if (maxKey < targetTotalSize + NUM_THREADS) {
                    ...
                    // Otherwise, it will be impossible to insert in the full sub-ranges after
                    // filling.
                }
                ...
                assert fill(set, targetTotalSize, maxKey, experimentRng, isSizeSupported, true) +
                        insertRemoveAlternately(set, targetTotalSize, maxKey, experimentRng, isSizeSupported,
                                true) == empty(set, targetTotalSize, targetTotalSize, maxKey, experimentRng,
                                        isSizeSupported);
                if (isSizeSupported)
                    assert set.size() == 0;
            }
    ...
    public static void runTests() {
        ...
        // set MAX_TRIES for testing the Optimistic methodology
        final int MAX_TRIES = 3;

        // This addition is required for the size methods to iterate over all threads' metadata
        // due to Optimization C.4.2 in the appendix.
        ThreadID.nextId.set(ThreadID.MAX_THREADS);
        ...
        for (SetFactory<Integer> set : Factories.factories) {
            if (set.getName().contains("Batch")) {
                for (int i = 0; i < setParam.length; i++) {
                    ...
                    AbstractAdapter<Integer> setAdapter = (AbstractAdapter<Integer>) set.newSet(setParam[i], MAX_TRIES);
                    runTests(setAdapter);
                    System.out.println();
                }
                ...
                for (int targetSizePerThread : TARGET_SIZES_PER_THREAD) {
                    ...
                    AbstractAdapter<Integer> setAdapter = (AbstractAdapter<Integer>) set.newSet(tableSize, MAX_TRIES);
                    runTests(setAdapter);
                    System.out.println();
                }
            } else {
                System.out.println("[*] Testing " + set.getName() + " ...");
                AbstractAdapter<Integer> setAdapter = (AbstractAdapter<Integer>) set.newSet(null, MAX_TRIES);
                runTests(setAdapter);
                System.out.println();
            }
        }
